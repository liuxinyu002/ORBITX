...
Define the visual Schema editor UI, field data structures, Zod validation rules, draft state management, and navigation guard for the `/tools/extractor` page. The editor provides tabular field editing with AI-assisted draft generation.
...
3. 校验通过 → 调用 `update_task(id, None, Some(description), Some(JSON.stringify({ fields })))` — name 传 None 保留原值
