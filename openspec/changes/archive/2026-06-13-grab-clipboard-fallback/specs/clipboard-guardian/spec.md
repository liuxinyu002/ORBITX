...
The restore mechanism SHALL call `pasteboard.clearContents()`, then immediately write `org.nspasteboard.TransientType` and `org.nspasteboard.ConcealedType` markers (each as an empty `NSData`), then write back all backed-up `(type, data)` pairs in their original order.
