...
description: Review an OpenSpec change across 6 dimensions. Use when the user wants to review a proposal before implementation, validate change quality, or check for issues before applying.
license: MIT
...
Output a structured review report using the exact format below.
...
- **Erring on the side of reporting.** When uncertain about severity, prefer Warning over silence. Goal: "宁可多报 Warning，不可漏报 Blocker".
...
/opsx:review [change-name]
    │
    ├─ Auto-detect change (or prompt user)
    │
    ├─ Read proposal/design/specs/tasks
    │
    ├─ Execute 6-dimension review
    │
    ├─ Output structured report
    │   ├─ Global summary + next-step actions
    │   └─ Expanded view (only failing dimensions)
    │
    ├─ If user authorizes fixes:
    │   ├─ Show modification preview
    │   ├─ Confirm scope (Blocker only / +Warning / one-by-one)
    │   ├─ Apply minimal fixes to source artifacts
    │   └─ Output change receipt
    │
    └─ If user keeps diagnosis: exit, no files changed
```
